...
- [] Add different draw modes first. I want to add a mode for listing: workouts, notes, and max lifts. Each bound to a key press.
    - [x] w can be workouts
    - [x] n can be notes
    - [x] m can be max lifts.
    - [] Implementation: I will want to separate each of these into their own files with their own rules on how to display each. I already have a workout.lua file handling all the workout graphics so thats good.
...
Done:
- [X] 20260101: Add some colors for background, box lines, box, and font. Added a color_scheme.lua file that can load a file with colors. Those colors are then used for the different drawables.
